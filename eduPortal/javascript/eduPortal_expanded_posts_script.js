...
function displayRecentPosts() {
    ...
    // TO DISPLAY ALL RECENT POSTS ON THE MAIN PAGE, UNDER "RECENT POSTS" SECTION
    for (let i = teacherTitle.length - 1; i >= 0; i--) {
...
function deletePost(index) {
    ...
    const teacherImages = JSON.parse(localStorage.getItem("teacherImagesKey")) || []; // RECALL TEACHER IMAGES FROM LOCAL STORAGE
    ...
    teacherImages.splice(index, 1); // REMOVE THE IMAGE AT THE SPECIFIED INDEX
    ...
    localStorage.setItem("teacherImagesKey", JSON.stringify(teacherImages)); // SAVE THE UPDATED TEACHER IMAGES BACK TO LOCAL STORAGE
